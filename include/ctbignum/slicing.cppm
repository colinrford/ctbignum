...
export module lam.ctbignum:slicing;
...
// skip<N>(x) corresponds with right-shifting x by N limbs
export template <typename T, std::size_t N1> constexpr auto skip(big_int<N1, T> t, std::size_t N)
{
...
export template <std::size_t N, typename T, std::size_t N1> constexpr auto first(big_int<N1, T> t)
{
  return take<0, N>(t);
}
...
export template <typename T, std::size_t N1> constexpr auto first(big_int<N1, T> t, std::size_t N)
{
  return take<N1>(t, 0, N);
}

// add N extra limbs (at msb side)
export template <std::size_t N, typename T, std::size_t N1> constexpr auto pad(big_int<N1, T> t)
{
  return take<0, N1, N>(t);
}

export template <std::size_t N, typename T, std::size_t N1> constexpr auto to_length(big_int<N1, T> t)
{
  return (N1 < N) ? pad<N - N1>(t) : first<N>(t);
}

export template <typename T, std::size_t N1, std::size_t N2> constexpr auto join(big_int<N1, T> a, big_int<N2, T> b)
{
...
export template <std::size_t K, std::size_t N, typename T = std::uint64_t> constexpr auto unary_encoding()
{
  big_int<N, T> res{};
  res[K] = 1;
  return res;
...
export template <std::size_t N, typename T = std::uint64_t> constexpr auto unary_encoding(std::size_t K)
{
  big_int<N, T> res{};
  res[K] = 1;
  return res;
...
export template <std::size_t N, typename T = std::uint64_t> constexpr auto place_at(T value, std::size_t K)
{
  big_int<N, T> res{};
  res[K] = value;
  return res;
