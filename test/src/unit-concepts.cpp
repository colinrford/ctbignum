...
concept multiplicative_group_element_c_weak = has_multiplicative_identity_c_weak<G> and requires(G g, G h, G k) {
  ...
  requires requires {
    { 1 / g } -> std::same_as<decltype(g)>;
  } or requires {
    { G::one() / g } -> std::same_as<decltype(g)>;
  } or requires {
    { inv(g) } -> std::same_as<decltype(g)>;
  };
};
 ...
 */
template<typename R>
concept ring_element_c_weak =
  additive_group_element_c_weak<R> and has_multiplicative_identity_c_weak<R> and requires(R r, R s, R t) {
 ...
 */
template<typename K>
concept field_element_c_weak = ring_element_c_weak<K> and multiplicative_group_element_c_weak<K>;
