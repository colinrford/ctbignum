...
TEST_CASE("Addition")
{
  ...
  constexpr big_int<2> correct_answer = {{18446744073709551435UL, 4611686018427387903UL}};
...
TEST_CASE("Barrett reduction")
{
  ...
  constexpr auto mu =
      to_big_int(8343699359066055009355553539724812947666814540455674882604411090793790119337922481889828929536_Z);
...
TEST_CASE("Division")
{
  ...
  constexpr auto u = to_big_int(
      4925250774549309901534880012517951725634967408808180833493536675530715221437151326426783281860614455100828498788859_Z);
...
TEST_CASE("Division where divisor is larger than dividend")
{
  ...
  static_assert(div(to_big_int(8712634_Z), to_big_int(18374598123740981237509874103891_Z)).remainder ==
                to_big_int(8712634_Z));
}
...
TEST_CASE("short div")
{
  ...
  constexpr auto quotrem = lam::cbn::short_div(a, b[0]);
...
TEST_CASE("gcd")
{
  ...
  constexpr auto gcd = lam::cbn::ext_gcd(a, b);
...
TEST_CASE("gcd easy") {
  ...
  constexpr auto gcd = lam::cbn::ext_gcd_(a,b);
...
TEST_CASE("modular inverse")
{
  ...
  static_assert(lam::cbn::mod_inv(x, m) == ans, "fail");
}
...
TEST_CASE("Modular Exponentiation")
{
  ...
  static_assert(lam::cbn::mod_exp(x, e, m) == ans, "fail");
  REQUIRE(lam::cbn::mod_exp(x, e, m) == ans);

  // static_assert(lam::cbn::mod_exp_montgomery(x,e,m) == ans, "fail");
  // REQUIRE(lam::cbn::mod_exp_montgomery(x,e,m) == ans);
}
